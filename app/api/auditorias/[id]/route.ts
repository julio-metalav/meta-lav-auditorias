...
export default function InternoAuditoriaPage({ params }: { params: { id: string } }) {
  ...
  const isFinal = useMemo(() => {
    const s = normStatus(aud?.status);
    return s === "final" || s === "finalizado";
  }, [aud?.status]);
  ...
  async function carregar() {
    ...
    try {
      ...
      const audRow: Aud = {
        id: String(a.id ?? a?.auditoria?.id ?? id),
        condominio_id: String(a.condominio_id ?? a?.auditoria?.condominio_id ?? ""),
        mes_ref: (a.mes_ref ?? a.ano_mes ?? a?.auditoria?.mes_ref ?? null) as any,
        status: a.status ?? a?.auditoria?.status ?? null,
        agua_leitura: a.agua_leitura ?? a.leitura_agua ?? a?.auditoria?.agua_leitura ?? null,
        energia_leitura: a.energia_leitura ?? a.leitura_energia ?? a?.auditoria?.energia_leitura ?? null,
        gas_leitura: a.gas_leitura ?? a.leitura_gas ?? a?.auditoria?.gas_leitura ?? null,
        base_agua: a.base_agua ?? a?.auditoria?.base_agua ?? null,
        base_energia: a.base_energia ?? a?.auditoria?.base_energia ?? null,
        base_gas: a.base_gas ?? a?.auditoria?.base_gas ?? null,
        condominios: a.condominios ?? a?.auditoria?.condominios ?? null,
        condominio: a.condominio ?? a?.auditoria?.condominio ?? null,
      };
  ...
  async function reabrirParaCorrecao() {
    if (!aud) return;
    if (!isStaff) return;

    const ok = window.confirm("Reabrir auditoria finalizada para correção?\n\nIsso vai mudar o status para EM CONFERÊNCIA e liberar edição.");
    if (!ok) return;
    ...
    try {
      const res = await fetch(`/api/auditorias/${aud.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status: "em_conferencia",
          note: "Reaberta para correção (interno/gestor)",
        }),
      });
      ...
      if (!ct.includes("application/json")) {
        const text = await res.text().catch(() => "");
        throw new Error(`/api/auditorias/${aud.id} retornou ${res.status} (não-JSON). Trecho: ${text.slice(0, 200)}`);
      }

      const json = await res.json();
      if (!res.ok) throw new Error(json?.error ?? "Falha ao reabrir auditoria");

      await carregar();
    } catch (e: any) {
      setErr(e?.message ?? "Erro inesperado ao reabrir auditoria");
    } finally {
      setReopening(false);
    }
  }
      ...
      <div className="mx-auto max-w-5xl px-6 py-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {isFinal && isStaff ? (
              <button
                className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-800 hover:bg-amber-100 disabled:opacity-50"
                onClick={reabrirParaCorrecao}
                disabled={reopening || loading}
              >
                {reopening ? "Reabrindo..." : "Reabrir para correção"}
              </button>
            ) : null}
        ...
        {isFinal ? (
          <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            Auditoria está <b>finalizada</b>. Edição de ciclos/consumo fica travada. Use <b>Reabrir para correção</b> se precisar ajustar.
          </div>
        ) : null}
            ...
            {ciclos.map((it, idx) => (
              <div key={it.maquina_tag} className="grid grid-cols-12 items-center px-4 py-3">
                ...
                <div className="col-span-3 flex justify-end">
                  <input
                    className="w-24 rounded-xl border border-gray-200 px-3 py-2 text-right text-sm disabled:bg-gray-50 disabled:opacity-70"
