...
function PhotoCard(props: { label: string; item: AnexoPdf | null; full?: boolean }) {
  ...
  return (
    <>
      ...
      {has ? (
        <Image src={imgDataUri(item!.src!)} style={full ? S.photoImgFull : S.photoImgHalf} />
      ) : (
        <View style={full ? S.photoMissingFull : S.photoMissing}>
...
export default function RelatorioFinalPdf(p: Props) {
  ...
  // 2 páginas SEMPRE: pegamos só as evidências que entram no relatório do síndico
  const aAgua = pickAnexo(anexosValidos, (t) => t.includes("agua"));
  const aEnergia = pickAnexo(anexosValidos, (t) => t.includes("energia"));
  const aGas = pickAnexo(anexosValidos, (t) => t.includes("gas"));
  const aComprovante = pickAnexo(anexosValidos, (t) => t.includes("comprov"));

  const temGas = !!aGas?.src?.data;

  return (
    <Document>
      {/* Página 1: Prestação de Contas (igual) */}
        ...
        {temGas ? (
          <>
            <View style={[S.row, S.rowMb]}>
              <View style={S.photoHalfLeft}>
                <PhotoCard label="Medidor de Água" item={aAgua} />
              </View>
              <View style={S.photoHalfRight}>
                <PhotoCard label="Medidor de Energia" item={aEnergia} />
              </View>
            </View>

            <View style={S.row}>
              <View style={S.photoHalfLeft}>
                <PhotoCard label="Medidor de Gás" item={aGas} />
              </View>
              <View style={S.photoHalfRight}>
                <PhotoCard label="Comprovante de pagamento" item={aComprovante} />
              </View>
            </View>
          </>
        ) : (
          <>
            ...
            <View style={S.photoFull}>
              <PhotoCard label="Comprovante de pagamento" item={aComprovante} full />
            </View>
          </>
        )}
